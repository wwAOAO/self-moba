function connect() {
    ...
    state.playerId = Array.from(els.playerId.value.trim() || state.playerId)
        .slice(0, 16)
        .join('');
...
function applySnapshot(snapshot) {
    ...
    state.players = new Map(players.map(player => [player.playerId, normalizePlayer(player)]));
    ...
    if (
        ...
        (!currentTargets.has(state.selectedTargetId) || currentTargets.get(state.selectedTargetId)?.dead)
    ) {
        state.selectedTargetId = '';
    }
    if (
        state.attackTargetId &&
        (!currentTargets.has(state.attackTargetId) || currentTargets.get(state.attackTargetId)?.dead)
    ) {
        state.attackTargetId = '';
    }
...
function showTargetDamage(id, target) {
    ...
    for (const event of damageEvents.slice(-maxDamageEventsPerTarget)) {
        ...
        spawnDamageText(target, event.damage || 0, event.damageType || 'physical', localDamage);
    }
}
...
function castSkill(slot) {
    ...
    if (skillId === 'crossbowman_tumble' && (self.buffs || []).some(buff => buff.id === skillId)) {
        return;
    }
    if (
    ...
    const hoveredSkillTarget = skillId === 'shadow_assassin_e' ? hoveredEnemyHero() : null;
    if (skillId === 'shadow_assassin_e' && !hoveredSkillTarget) {
        return;
    }
    const selected = hoveredSkillTarget || currentTarget();
    ...
    sendPacket('input', {
        cast: {
            skillId,
            targetId: skillId === 'slam' || skillId === 'trap' || cursorOnlyTargeting ? '' : selected?.id || '',
            targetX: target.x,
            targetY: target.y,
        },
        clientSeq: state.seq,
...
function addCastWindup(self, skillId, target, selectedTarget) {
    ...
    if (skillId.startsWith('berserker_') || skillId.startsWith('fire_mage_') || skillId.startsWith('sword_')) {
        return;
    }
    const windupSeconds = Number(config.castWindupSeconds || 0) || Number(config.castDelaySeconds || 0);
    if (windupSeconds <= 0) {
        return;
    }
...
function hasActiveCastWindup(skillId, now) {
    return state.castWindups.some(windup => windup.skillId === skillId && now <= windup.expiresAt);
}
...
function debugLevelUp() {
    const self = state.players.get(state.playerId);
    if (!self || self.dead || self.level >= (self.maxLevel || levelClientConfig.maxLevel || 18)) {
        return;
    }
    sendPacket('input', {
        debugLevelUp: true,
...
function toggleDebugAbilityHaste() {
    ...
    const enabled = (self.buffs || []).some(buff => buff.id === 'debug_ability_haste');
    sendPacket('input', {
