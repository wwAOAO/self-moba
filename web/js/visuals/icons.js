/** 将服务端碰撞半径转换为当前镜头下的屏幕半径。 */
function playerModelRadius(player) {
    return Math.max(1, (player.radius || 18) * state.frame.scale);
}
...
function playerModelShape(player) {
    ...
    if (player.heroId === 'shadow_assassin') {
...
function drawKatanaIcon(graphics, radius) {
    ...
    graphics.quadraticCurveTo(-radius * 0.32, radius * 0.48, -radius * 0.86, radius * 0.82);
    graphics.quadraticCurveTo(-radius * 1.12, radius * 0.98, -radius * 1.24, radius * 0.86);
    graphics.quadraticCurveTo(-radius * 1.12, radius * 1.18, -radius * 0.74, radius * 1.12);
    graphics.quadraticCurveTo(-radius * 0.08, radius * 0.86, radius * 0.55, radius * 0.08);
...
function drawGunnerIcon(graphics, radius) {
    graphics.roundRect(-radius * 1.5, -radius * 0.95, radius * 3.15, radius * 0.7, radius * 0.08);
...
function drawKillerDagger(graphics, radius, side) {
    ...
    graphics.quadraticCurveTo(mirrorX(radius * 0.72), -radius * 0.84, mirrorX(radius * 0.2), -radius * 0.12);
    graphics.lineTo(mirrorX(radius * 0.48), radius * 0.12);
    graphics.quadraticCurveTo(mirrorX(radius * 0.94), -radius * 0.46, mirrorX(radius * 1.34), -radius * 1.16);
    graphics.closePath();
...
function drawShurikenBlade(graphics, radius, angle) {
    ...
    graphics.quadraticCurveTo(leftInner.x, leftInner.y, pointAt(angle, radius * 0.5).x, pointAt(angle, radius * 0.5).y);
...
function drawBerserkerIcon(graphics, radius) {
    ...
    graphics.quadraticCurveTo(radius * 1.28, -radius * 0.9, radius * 1.16, -radius * 0.12);
...
function drawRobotIcon(graphics, radius) {
    graphics.roundRect(-radius * 0.72, -radius * 0.62, radius * 1.44, radius * 1.18, radius * 0.16);
...
/** 将单位世界半径转换为当前镜头下的屏幕半径。 */
function unitModelDisplayRadius(unit) {
    return Math.max(1, (unit.radius || 18) * state.frame.scale);
}
