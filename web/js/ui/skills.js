function updatePositionLabel() {
    ...
    if (!els.buffs.querySelector('.stat-tip:hover')) {
...
function formatSkillCooldowns(player, tick) {
    const slots = heroSkillSlots[player.heroId || els.heroId.value] || heroSkillSlots[els.heroId.value];
    if (!slots) {
        return '-';
    }
...
function formatPassiveSkillRow(player, skillId, tick) {
    ...
    const remainTicks = Math.max(0, (skill?.cooldownUntilTick || 0) - tick);
    const timeText = remainTicks > 0 ? `${(remainTicks / state.tickRate).toFixed(1)}s` : '-';
...
function formatSkillTipValue(key, value) {
    ...
    if (key.includes('Ratio') || key.includes('Bonus') || key.includes('Percent') || key === 'slow') {
...
function formatArcherSkillState(player, tick) {
    ...
    if ((archer.focusActiveUntil || 0) > tick) {
        const remain = ((archer.focusActiveUntil - tick) / state.tickRate).toFixed(1);
...
function skillIdForSlot(heroId, slot) {
    return heroSkillSlots[heroId]?.[slot] || heroClientConfig[heroId]?.skills?.[slot] || '';
}

function skillState(player, skillId) {
    return (player.skills || []).find(skill => skill.skillId === skillId) || null;
}
