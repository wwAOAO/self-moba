...
func SunfireDamage(level int, minLevel int, maxLevel int, bonusHP float64, item config.EquipmentConfig, equipped state.EquipmentSlot) float64 {
	level = formula.ClampInt(level, minLevel, maxLevel)
	...
	return (base + bonusHP*item.Effects.SunfireBurnBonusHPRatio) * (1 + equipped.Stacks*item.Effects.SunfireStackDamageBonus)
}
